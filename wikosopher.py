...
def get_to_philosophy(url, cache=[]):
    ...
    # names of certain classes and ids to be removed
    # (red, external, /link/, [link], footnote, country coordinates)
    ...
    # find all paragraphs in the needed class,
    # exclude all unwanted classes and ids,
    # remove italic tags and parentheses
    for p in soup.find(class_='mw-parser-output').find_all('p', recursive=False):
        ...
        for italic in p.find_all('i'):
            italic.extract()
        # continue untill finding the first link in an edited paragraph
        # and only then toggle the flag off
        try:
    ...
    # the fuction calls itself over again untill getting to philosophy
    # or detecting a loop from a previous link
